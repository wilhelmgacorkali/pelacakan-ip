<?php $__env->startSection('title', 'Dashboard Pelacak IP & Nomor Telepon'); ?>
...
<div class="space-y-8">
    ...
    <div class="flex flex-col sm:flex-row items-center justify-between gap-4 border-b border-slate-800 pb-4">
        ...
        <div class="text-xs text-slate-400 flex items-center space-x-2 self-end sm:self-center">
            ...
            <span class="font-mono px-2 py-0.5 rounded bg-slate-800 border border-slate-700 text-indigo-300 font-semibold"><?php echo e($clientIp); ?></span>
        </div>
    </div>
    ...
    <?php if(isset($recentSearches) && count($recentSearches) > 0): ?>
    <div class="glass-panel p-6 rounded-2xl space-y-4">
        <div class="flex items-center justify-between">
            ...
            <a href="<?php echo e(route('tracker.history')); ?>" class="text-xs text-indigo-400 hover:text-indigo-300 transition-colors font-medium">
                Lihat Semua (<?php echo e(count($recentSearches)); ?>+) &rarr;
            </a>
        </div>

        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
            <?php $__currentLoopData = $recentSearches; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="p-3 bg-slate-900/80 rounded-xl border border-slate-800/80 hover:border-indigo-500/30 transition-all flex items-start space-x-3">
                    <div class="mt-0.5 w-7 h-7 rounded-lg <?php echo e($item->type === 'ip' ? 'bg-indigo-500/20 text-indigo-400' : 'bg-emerald-500/20 text-emerald-400'); ?> flex items-center justify-center text-xs">
                        <i class="fa-solid <?php echo e($item->type === 'ip' ? 'fa-globe' : 'fa-phone'); ?>"></i>
                    </div>
                    <div class="flex-1 min-w-0">
                        <div class="font-mono text-xs font-bold text-white truncate"><?php echo e($item->query); ?></div>
                        <div class="text-[11px] text-slate-400 truncate"><?php echo e($item->title ?? '-'); ?></div>
                        <div class="text-[10px] text-slate-500 mt-1"><?php echo e($item->created_at->diffForHumans()); ?></div>
                    </div>
                </div>
...
<script>
    ...
    async function performIpLookup(ip) {
        ...
        try {
            ...
            const res = await fetch('<?php echo e(route("api.track.ip")); ?>', {
                method: 'POST',
                headers: {
    ...
    async function performPhoneLookup(phone) {
        ...
        try {
            const csrfToken = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
            const res = await fetch('<?php echo e(route("api.track.phone")); ?>', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': csrfToken,
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\IP\resources\views/tracker/index.blade.php ENDPATH**/ ?>